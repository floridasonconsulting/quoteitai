...
export function IntegrationsSection() {
  return (
    <section id="integrations" className="py-24 bg-[#0A0A0A] relative overflow-hidden">
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {integrationBenefits.map((integration, idx) => (
            ...
            >
              <div className="h-14 w-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center mb-6 group-hover:scale-110 group-hover:bg-primary/10 transition-all duration-500">
                <integration.icon className="h-7 w-7 text-primary" />
              </div>
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-white tracking-tight">{integration.title}</h3>
                <Badge variant="outline" className="border-white/10 text-slate-400 text-[10px] uppercase font-bold px-2">
                  {integration.badge}
                </Badge>
              </div>
              <ul className="space-y-4">
                {integration.features.map((feature, fidx) => (
                  <li key={fidx} className="flex items-start gap-3 text-sm text-slate-400 group-hover:text-slate-200 transition-colors">
