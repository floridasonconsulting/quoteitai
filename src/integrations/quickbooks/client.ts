...
export class QuickBooksClient {
  ...
  async sendInvoice(invoiceId: string, email: string): Promise<void> {
    await this.request<any>(`invoice/${invoiceId}/send?sendTo=${email}`, {
      method: "POST",
    });
  }
  ...
  async createPayment(payment: QuickBooksPayment): Promise<QuickBooksPayment> {
    const response = await this.request<any>("payment", {
      method: "POST",
  ...
  async createItem(item: QuickBooksItem): Promise<QuickBooksItem> {
    const response = await this.request<any>("item", {
      method: "POST",
  ...
  async getCompanyInfo(): Promise<QuickBooksCompanyInfo> {
    const response = await this.request<any>("companyinfo/" + this.realmId);
